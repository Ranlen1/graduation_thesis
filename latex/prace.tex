...
\pagenumbering{roman}                   % číslování stránek římskými číslicemi
	...
	\abstrakt
    {
       Tato práce se zaměřuje na základní vysvětlení počítačové grafiky. Zaobírá se architekturou
       grafických karet a fungováním grafické zobrazovací pipeline. Dále popisuje fungování knihovny
       OpenGL a jazyky C a C++, ve kterých se s ní často pracuje. V praktické části jsem
...
\chapter*{Úvod}     % úvod práce 
    ...
    vývoj videoher nebo aplikací s grafickým uživatelským rozhraním.
    Většina programátorů využívá pro vývoj vysoce abstraktní knihovny nebo herní enginy jako
    Pygame, Unity či Unreal Engine. Vytváření výsledného produktu je výrazně 
...
%%%%%%%%%%%%%% TEORETICKÁ ČÁST %%%%%%%%%%%%%%%%%%	
\part{Teorie pro vývoji hry v OpenGL}  % název teoretické části (nenechávejte Teoretická část)
	
\chapter{Programovací jazyky}
			
\section{Programovací jazyk C}
    C je středněúrovňový programovací jazyk. Je podobou blízko strojovému kódu,
...
\subsection{CPU vs. GPU}
    ...
    dostatečně výkonné na složíté výpočty, a proto už v 70. a 80.
    letech začaly vznikat počítače, které nepracovali sekvenčně, ale paralelně. V 90.
    letech se začali vytvářet mikroprocesory, které se soustředily na paralelní výpočty (\gls{gpu}).
    ...
    se stala součástí většiny počítačů. \gls{gpu} a \gls{cpu} se používá v různých případech kvůli
    ...
    Velká část čipu je určena pro cache, tedy malou pamět, kam se ukládají data, která budou 
    ...
    přístupem do cache
    výrazně rychlejší. Narozdíl od toho má \gls{gpu} stovky až tisíce jader.
    Jednotka řídící logiku a cache jsou v jádru menší a jednodužší, 
...
\subsection{Architektura a paměťová hierarchie GPU}
    ...
    Většina grafických karet má svoji vlastní dynamickou \gls{ram} (\gls{dram}), ke které mají přístup
    ...
    Jedním takovým typem pamětí je sdílená paměť, která má většinou desítky
...
\section{Grafická zobrazovací pipeline}
    ...
    potřeby vytvářet nové vrcholy z existujících trojúhelníků nebo bodů, což se používá například u
    malých částic, aby byly lépe vidět na obrazovce, takže se z nich udělá například malý čtverec.
...
OpenGL je považováno za aplikační programovcí rozhraní (\gls{api}), které uživatelům poskytuje sadu
funkcí, kterou mohou používat k manipulaci s grafickou kartou. Specifikaci pro OpenGL tvoří
...
základní shadery, aby \gls{gpu} něco vykreslilo. Shadery jsou od sebe izolované a komunikují spolu
...
ukládají do vztupních dat spolu s pozicemi vrcholů.
...
\chapter{to do}
    ...
    pouziti texttt funkce pro nazev funkci a podobnych veci
...
\begin{itemize}
    ...
    \item \texttt{game\_running.cpp} – Renderuje stav běžící hry a volá funkce pro objekty ve hře.
    ...
    \item \texttt{random.cpp} – Vytváří náhodné čísla typu double a float.
\end{itemize}
...
číslem, čímž vytvoří číslo v rozmezí od 0 do 1. Následně se rozmezí, které je potřeba vytvořit,
přemapuje na rozmezí od 0 do 1.
\cite{cppreference}
...
\end{lstlisting}
FruitType slouží k tomu, aby se přiřadila správná textura k danému ovoci a aby se vedělo, jaký efekt
nastane při kolizi. Ve fruitAttributes jsou všechny potřebné parametry pro každé ovoce pohromadě,
...
V druhé části práce je popsána vytvořená hra v jazyce C++. Hra využívá teorii z předchozí kapitoly
a základní OpenGL funkce jako práce s shadery a texturami, převádění souřadnic skrz různé souřadnicové
