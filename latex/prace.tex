...
\subsection{CPU vs. GPU}
    ...
    dostatečně výkonné na složíté výpočti, a proto se už sedmdesátých a osmdesátých
    letech začali vznikat počítače které nepracovali sekvenčně, ale paralelně. V devadesátých se
    začali vytvářet mikroprocesory, které se soustředili na paralelní výpočty \gls{gpu}. V průběhu
    let se počet vláken z jednotek dostal na tisíce, což vytvořilo prostor pro inovaci v grafice.
    ...
    menší propustnost paměti než \gls{gpu}, proto cache-friendly programy jsou výrazně rychlejší.
    Narozdíl od toho \gls{gpu} má stovky až tisíce jader, která jsou ješte typicky rozdělena na 32
    nebo 64 vláken. Kontrolní logika a cache jsou na jádro menší a jednodužší, 
    takže velmi pomalu reaguje na
    události a po dokončení operaí se většinou data vrací zpět do \gls{vram}, což je díky velké
    propustnosti i pro velmi velký objem dat rychlé.
    \cite{programming_massively_parallel_processors}

\subsection{Paralelní architektura GPU}
    Do roku 2007 se \gls{gpu} pouzival prevazne pro renderovani, ale v roce 2007 prisla NVIDIA s
    programovacim modelem pro paralelni vypocty CUDA. Tento model umoznil spousteni aplikaci
    na \gls{cpu} a \gls{gpu}, takze narocnejsi pocetni operace mohly bezet na \gls{gpu}, a tim
    zvysit vykon. Pro ucel vysvetleni programovacich modelu pro paralelni vypocty budu vyuzivat
    model CUDA, ale existuji i jine, podobne fungujici modely jako OpenCL nebo SYCL.

    \begin{figure}
        \includegraphics[width=\linewidth]{GPU_architecture.pdf}
        \caption{Architektura GPU podporující CUDA \cite{programming_massively_parallel_processors}}
    \end{figure}
...
\chapter{to do}
    ...
    části o gpu, a nejspis to bude potreba zkratit.
