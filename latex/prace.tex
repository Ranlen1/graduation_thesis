...
\section{Historie a architektura grafických karet}
    ...
    Small-Scale Experimental Machine(SSEM). Tento počítač byl vytvořen v roce 1948 na Manchesterské
    ...
    běžet letecký simulátor. Tak tedy v roce 1944 začal Jay Forrester pracovat v laboratořích MIT
    na projektu Whirlwind. Při práci vyvinul Forrester paměť s náhodným přístupem(\gls{ram}),
    tvořenou magnetickými jádrami, skrz které proudil koindidenční proud. Počítač byl dokončen
    v roce 1951. Využíval \gls{crt} podobně jako baby a dokázal na svém displeji řešit rovnice,
    ...
    pomocí radarů dokázal odhalovat letadla a řídit obrané síly proti případným letadlům. Projekt
    SAGE byl jeden z prvních systému, které využívaly interaktivní ovládání pomocí klávesnice či
    speciálního laserového pera. Pokud bylo světlem z tohoto pera posvíceno na ikonu letadla, počítač
    zobrazil informace o daném letadle jako rychlost a směr jeho letu.
    \cite{history_of_the_gpu_new_developments, milestones:whirldwind_computer, sage:semi-automatic_ground_environment_air_defense_system}
